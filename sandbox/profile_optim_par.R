...
#'
#' @returns A data.frame with the profile for each parameter
#' @examples
#' optimVector <- mll_df_example[1, 1:9] |> unlist()
#' profile_optim_par(optimVector, stepsize = 1)
profile_optim_par <- function(optim_vec,
                              envdat, 
                              occ,
                              stepsize = 0.001,
                              parallel = FALSE) {
  f_optim_functor <- function(envdat, pa) {
    function(optim_vec,
      ...
      optim_ll <- loglik_math(optim_vec,
        envdat,
        occ,
        negative = FALSE
      )
      ...
      alpha_step <- 0.94
      alpha <- 0.95
      degree_freedom <- 1 # number of fixed parameters
